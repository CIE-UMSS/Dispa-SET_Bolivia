...
# Función para construir la matriz de susceptancias Bbus
def build_Bbus(feeder, slack_bus):
    # Número de nodos y líneas
    nodes = list(set(feeder['Node_i']).union(set(feeder['Node_j'])))
    n = len(nodes)
    
    # Crear una matriz de susceptancias vacía
    Bbus = np.zeros((n, n), dtype=complex)

    # Rellenar la matriz Bbus usando los datos de la red
    for _, row in feeder.iterrows():
        ni = int(row['Node_i']) - 1  # Restar 1 para índice de Python (empezar en 0)
        nj = int(row['Node_j']) - 1
        x = row['X']
        b = 1 / (1j * x)
    ...
    # Ajustar la fila y columna del nodo slack
    Bbus[slack_bus, :] = 0
    Bbus[:, slack_bus] = 0
    Bbus[slack_bus, slack_bus] = 1
...
# Función para calcular la matriz PTDF
def calculate_PTDF(feeder, Bbus, slack_bus):
    # Número de nodos y líneas
    n = Bbus.shape[0]
    l = len(feeder)

    # Invertir la matriz Bbus
    X = np.linalg.inv(Bbus)

    # Crear la matriz de incidencia nodos-líneas
    A = np.zeros((n, l))
    ...
    # Calcular la matriz PTDF
    Bd = np.diag([1 / (1j * feeder['X'][i]) for i in range(l)])
    PTDF = Bd @ A.T @ X

    # Eliminar la columna correspondiente al nodo slack
    PTDF = np.delete(PTDF, slack_bus, axis=1)
...
slack_bus = int(Slack.iloc[0, 0]) - 1  # Restar 1 para índice de Python

# Construir la matriz Bbus
Bbus = build_Bbus(feeder, slack_bus)

# Calcular la matriz PTDF
PTDF = calculate_PTDF(feeder, Bbus, slack_bus)

# Guardar la matriz PTDF en un archivo CSV
ptdf_df = pd.DataFrame(PTDF, index=feeder['Line'], columns=feeder['Node_i'].unique())
ptdf_df.to_csv('PTDF.csv')

print("Matriz PTDF calculada y guardada en 'PTDF.csv'")
