...
plt.figure(figsize=(15,7))
sns.lineplot(data = df0, y='Inertia_S0', x = 'TIMESTAMP' , linewidth=1, color='teal',alpha=1,label='Inertia_EPI-BL',n_boot=100)
sns.lineplot(data = df1, y='Inertia_S1', x = 'TIMESTAMP' , linewidth=1, color='darkslategrey',alpha=1,label='Inertia_EPI-DC',n_boot=100)
sns.lineplot(data = df2, y='Inertia_S2', x = 'TIMESTAMP' , linewidth=1, color='navy',alpha=1,label='Inertia_EPI-SC',n_boot=100)
sns.lineplot(data = df3, y='Inertia_S3', x = 'TIMESTAMP' , linewidth=1, color='seagreen',alpha=1,label='Inertia_NZE-BL',n_boot=100)
sns.lineplot(data = df4, y='Inertia_S4', x = 'TIMESTAMP' , linewidth=1, color='darkkhaki',alpha=1,label='Inertia_NZE-DC',n_boot=100)
sns.lineplot(data = df5, y='Inertia_S5', x = 'TIMESTAMP' , linewidth=1, color='plum',alpha=1,label='Inertia_NZE-SC',n_boot=100)
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.,fontsize = 22)
plt.xticks(fontsize = 20)
plt.yticks(fontsize = 20)
...
Inertia_S0 = np.sort(df0['Inertia_S0'])
Inertia_S1 = np.sort(df1['Inertia_S1'])
Inertia_S2 = np.sort(df2['Inertia_S2'])
Inertia_S3 = np.sort(df3['Inertia_S3'])
Inertia_S4 = np.sort(df4['Inertia_S4'])
Inertia_S5 = np.sort(df5['Inertia_S5'])
...
palette = ['teal','darkslategrey','navy', 'seagreen', 'darkkhaki', 'plum','plum']
...
# sns.stripplot(x='Variable', y='Accuracy', palette=palette,
              # alpha=0.1, data=melted_df)

ax.legend_.remove()  # remove the legend, as the information is already present in the x labels
plt.xticks(fontsize = 20)
plt.yticks(fontsize = 20)
ax.set_xlabel('Different Scenarios', fontsize=20)  # remove unuseful xlabel ('Variable')
