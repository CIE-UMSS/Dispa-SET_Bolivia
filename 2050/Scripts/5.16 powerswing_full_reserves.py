...
activation_times = {
    ...
    "pfr": dict(prep=6, ramp=36, delivery=36, deact=901),
    "afrr": dict(prep=36, ramp=306, delivery=306, deact=901),
...
def optimize_search(limit_freq=0.8, limit_rocof=0.5, limit_freq_steady_state=0.2):
    ...
    H_range = (1, 99)
    H_step = 1
    reserve_range = (0, Contingency_val*10)
    reserve_range1 = (0, Contingency_val*10)
    reserve_range2 = (0, Contingency_val*2)
    ...
    for H_val, FFR_val, PFR_val in PFR_candidates:
        ...
        while low <= high:
            t = activation_times["afrr"]["deact"] + 0.01
            mid = (low + high)/2
            max_fd, max_r, results_df = run_simulation(t, H_val, FFR_val, PFR_val, mid, 0)
            # Ventana de tiempo aFRR
            freq_window = results_df.loc[results_df['Time [s]'] < activation_times["afrr"]["delivery"] + 0.01]
            # Calcular máximo absoluto de cada columna
            max_fd = freq_window['Frequency Deviation [Hz]'].abs().max()
            max_r = freq_window['RoCoF [Hz/s]'].abs().max()
            # Filtrar por tiempos mayores a 60 s
            ...
            freq_steady_state = freq_after_60.max()
            if (max_fd <= limit_freq) and (max_r <= limit_rocof) and (freq_steady_state <= limit_freq_steady_state): # 
                aFRR_candidates.append((H_val, FFR_val, PFR_val, mid))
                high = mid - reserve_step
            else:
                low = mid + reserve_step

    if not aFRR_candidates:
        print("No se encontró ninguna combinación H+FFR+PFR+aFRR válida.")
        return None, None
    
    print(f"Combinaciones H+FFR+PFR+aFRR válidas: {aFRR_candidates}")    

    # --- Paso 4: mFRR ---
    all_candidates = []
    for H_val, FFR_val, PFR_val, aFRR_val in aFRR_candidates:
        t = activation_times["mfrr"]["ramp"] + 50
        max_fd, max_r, results_df = run_simulation(t, H_val, FFR_val, PFR_val, aFRR_val, Contingency_val)
        # Ventana de tiempo PFR
        freq_window = results_df.loc[results_df['Time [s]'] < activation_times["pfr"]["delivery"] + 0.01]
        # Calcular máximo absoluto de cada columna
        max_fd = freq_window['Frequency Deviation [Hz]'].abs().max()
        ...
        # freq_steady_state = freq_after_60.max()
        if (max_fd <= limit_freq) and (max_r <= limit_rocof): #and (freq_steady_state <= limit_freq_steady_state)
            all_candidates.append((H_val, FFR_val, PFR_val, aFRR_val, Contingency_val))
