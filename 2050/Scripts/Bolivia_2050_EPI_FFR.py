...
config = ds.load_config('../ConfigFiles/Config_BOLIVIA_2050_EPI_FFR.xlsx')
...
inputs,results = ds.get_sim_results(path='../Simulations/BOLIVIA_2050_EPI_FFR',cache=False)
...
inputs_MTS, results_MTS = ds.get_sim_results(path='../Simulations/BOLIVIA_2050_EPI_FFR', cache=False, inputs_file='Inputs_MTS.p',results_file='Results_MTS.gdx')
       ...
       #######################################################################
        ...
        if found_solution:
           ...
           ####################################################################### 
            ...
            ax2.set_ylim(-700, 700)   # Asegura la misma escala proporcional entre ambos
...
#%%
# Save the results of the frequency security constraints analisys
summary.to_csv('../Simulations/BOLIVIA_2050_EPI_FFR/Summary.csv', index=False)
Contingency.to_csv('../Simulations/BOLIVIA_2050_EPI_FFR/Contingency.csv', index=False)
with pd.ExcelWriter('../Simulations/BOLIVIA_2050_EPI_FFR/frec_sec_const.xlsx', engine='xlsxwriter') as writer:
...
# #%% PARA ENCONTRAR TOT GEN BAT
# batunits = inputs['units']

# fuels=['OTH']

# batunits = batunits[batunits.Fuel.isin(fuels)]

# batlist = list(batunits['Unit'])

# batoutputpow = results['OutputPower']

# batoutputpow = batoutputpow.T

# batoutputpow = batoutputpow.reset_index()

# batoutputpow.rename(columns = {'index':'Unit'}, inplace = True)

# batoutputpow = batoutputpow[batoutputpow.Unit.isin(batlist)]

# batoutputpow = batoutputpow.T

# batoutputpow.columns = batoutputpow.iloc[0]

# batoutputpow = batoutputpow[1:]

# batoutputpow['TRG'] = batoutputpow.sum(axis=1)

# batoutputpow['TG'] = results['OutputPower'].sum(axis=1)
