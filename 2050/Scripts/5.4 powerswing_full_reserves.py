...
# ============================
def optimize_search(limit_freq=0.8, limit_rocof=0.5, limit_freq_steady_state=0.2, maxiter=30, popsize=8):
    """
    ...
    def objective(x):
        ...
        max_fd, max_r, results_df = run_simulation(FFR_cap, PFR_cap, aFFR_cap, mFFR_cap, RR_max, H_val)
...
if __name__ == "__main__":
    ...
    ax2.plot(best_df['Time [s]'], best_df['mFFR_total [MW]'], label='mFFR_total')
    ax2.fill_between(best_df['Time [s]'], best_df['mFFR_total [MW]'], 0, where=best_df['mFFR_total [MW]'] > 0, color='red', alpha=0.2)
