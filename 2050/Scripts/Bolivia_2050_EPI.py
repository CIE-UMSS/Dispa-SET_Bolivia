...
inputs,results = ds.get_sim_results(path='../Simulations/BOLIVIA_2050_EPI',cache=False)
...
inputs_MTS, results_MTS = ds.get_sim_results(path='../Simulations/BOLIVIA_2050_EPI', cache=False, inputs_file='Inputs_MTS.p',results_file='Results_MTS.gdx')
...
rng1 = pd.date_range('2050-01-01', '2050-01-07', freq='D')
...
for combination in all_combinations:
    ...
    results1.append((system_inertia, system_gain))

# Sort results by SystemInertia
sorted_results = sorted(results1, key=lambda x: x[0])
...
with pd.ExcelWriter('power_swing_results.xlsx', engine='xlsxwriter') as writer:
    ...
    for index, row in Contingency.iterrows():
        ...
        lower_bound_H = 1
        upper_bound_H = 46  # Adjust the upper bound as needed
        # lower_bound_k = 1
        # upper_bound_k = 85  # Adjust the upper bound as needed
        lower_bound_k = 50
        upper_bound_k = 2650  # Adjust the upper bound as needed
        step1 = 1
        # step2 = 1
        step2 = 50
        ...
        # for k in range(lower_bound_k, upper_bound_k + 1, step2):
        for H in range(lower_bound_H, upper_bound_H + 1, step1):
            # for H in range(lower_bound_H, upper_bound_H + 1, step1):      # k variable
            for k in range(lower_bound_k, upper_bound_k + 1, step2):    
        ...
        if found_solution:
            ...
            print(f"Minimum k that satisfies constraints: {min_k}")# k variable
            print(f"Minimum H that satisfies constraints: {min_H}")
            # Update the DataFrame with the new values
            Contingency.at[index, 'k'] = k  # Store in 'k'
            Contingency.at[index, 'min_H'] = min_H  # Store in 'min_H'
            ...
            primaryreserve = np.where(t < 5, 0, ((-k * f) * (t-time_preparation)))
            ...
            results_df['min_K[GW/Hz]'] = min_k
            results_df['min_H[GW*s]'] = min_H
            ...
            # ax1.set_ylabel('Frequency [Hz]', color='tab:blue', fontsize=16)
            ...
            # ax2.plot(t, contingency, color='tab:red', linestyle='--', label='Contingency')
            # ax2.plot(t, primaryreserve, color='tab:purple', linestyle='--', label='PrimaryReserve')
            # ax2.set_ylabel('Power [MW]', color='tab:red', fontsize=16)
...
summary = pd.DataFrame(columns=['TIMESTAMP', 'Contingency[MW]', 'Inertia[GW*s]','PrimaryReserve[MW]', 'Gain[GW/Hz]'])
...
for nombre_df, df in freq_security.items():
    ...
    primer_contingency = df['Contingency[MW]'].iloc[1]  # Primer valor de la columna 'Contingency[MW]'
    ...
    max_primaryreserve = df['PrimaryReserve[MW]'].iloc[60]  # Valor en el que se estabiliza 'PrimaryReserve[MW]'
    ...
    new_row = pd.DataFrame({'TIMESTAMP': primer_timestamp, 'Contingency[MW]': primer_contingency,
                            'Inertia[GW*s]': primer_inertia, 'PrimaryReserve[MW]': max_primaryreserve, 'Gain[GW/Hz]': primer_gain}, index=[0])
...
#%%
# Save the results of the frequency security constraints analisys
summary.to_csv('../Simulations/BOLIVIA_2050_EPI/Summary.csv', index=False)
Contingency.to_csv('../Simulations/BOLIVIA_2050_EPI/Contingency.csv', index=False)
with pd.ExcelWriter('../Simulations/BOLIVIA_2050_EPI/frec_sec_const.xlsx', engine='xlsxwriter') as writer:
