...
config = ds.load_config('../ConfigFiles/Config_BOLIVIA_2050_EPI_DI_FFR_TEST.xlsx')

# # Limit the simulation period (for testing purposes, comment the line to run the whole year)
config['StartDate'] = (2050, 8, 1, 0, 0, 0)
config['StopDate'] = (2050, 8, 3, 0, 0, 0)
...
inputs,results = ds.get_sim_results(path='../Simulations/BOLIVIA_2050_EPI_DI_FFR_TEST',cache=False)
...
inputs_MTS, results_MTS = ds.get_sim_results(path='../Simulations/BOLIVIA_2050_EPI_DI_FFR_TEST', cache=False, inputs_file='Inputs_MTS.p',results_file='Results_MTS.gdx')
...
rng = pd.date_range('2050-01-01', '2050-01-03', freq='H')
for i in config['zones']:
    ds.plot_zone(inputs, results, z=i, rng=rng)
    
rng1 = pd.date_range('2050-01-01', '2050-01-03', freq='D')
