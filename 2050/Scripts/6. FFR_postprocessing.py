...
for nombre, ruta in escenarios.items():
    ...
    # df = results['OutputFFR_Available'].copy().to_frame(name='OutputFFR_Available')
    
    # Sumar todas las columnas de FFR por fila para obtener una sola serie
    ffr = results['OutputFFR_Available'].sum(axis=1).to_frame(name='FFR_Available')
    ...
    if ffr.empty:
        ...
        df = pd.DataFrame({'FFR_Available': np.zeros(len(time_index))}, index=time_index)
    else:
        # Si no está vacío, asegurar que sea DataFrame con el nombre correcto
        df = ffr.copy()
        if isinstance(df, pd.Series):
            df = df.to_frame(name='FFR_Available')
        elif isinstance(df, pd.DataFrame):
            df.columns = ['FFR_Available']  # Asegura el nombre correcto de la columna
    ...
    # Renombrar la columna '0' a 'FFR_Available'
    df = df.rename(columns={df.columns[0]: 'FFR_Available'})
...
# Ahora df_concat ya tiene columnas: ['index', 'FFR_Available', 'Fecha', 'Hora', 'Periodo', 'Escenario']

#%%
# === 1. Annual total FFR_Available by scenario ===

# Generar orden deseado por escenario (S0 a S6) y enfoque (NC, DC, SC)
orden_escenarios = ['EPI_NC', 'EPI_DC', 'EPI_SC', 'NZE_NC', 'NZE_DC', 'NZE_SC']

# Agrupar y ordenar
total = df_concat.groupby('Escenario')['FFR_Available'].sum().reset_index()
...
bars = ax.bar(total['Escenario'], total['FFR_Available'],
...
for bar in bars:
    ...
    ax.text(bar.get_x() + bar.get_width()/2, height + 0.02 * total['FFR_Available'].max(),
            f'{height:,.0f}', ha='center', va='bottom', fontsize=9)

# Estética del gráfico
ax.set_xlabel('Scenario', fontsize=12)
ax.set_ylabel('Total FFR (MW)', fontsize=12)
ax.set_title('Annual Total FFR by Scenario', fontsize=14, fontweight='bold')
...
#%%
# === 1.1 Annual total FFR_Available by scenario con curvas tendencia ===
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
...
for bar in bars:
    h = bar.get_height()
    ax.text(bar.get_x() + bar.get_width() / 2, h + 0.02 * total['FFR_Available'].max(),
            f'{h:,.0f}', ha='center', va='bottom', fontsize=9)

# === Curvas de tendencia ===
def plot_line(grupo, color, linestyle):
    y = grupo['FFR_Available'].values
...
def annotate_percent_change(grupo, base_escenario):
    base_val = grupo.loc[grupo['Escenario'] == base_escenario, 'FFR_Available'].values[0]
    for _, row in grupo.iterrows():
        if row['Escenario'] != base_escenario:
            delta = (row['FFR_Available'] - base_val) / base_val * 100
            bar = bars_dict[row['Escenario']]
            x = bar.get_x() + bar.get_width() / 2
            y = bar.get_height()
            ax.text(x, y + 0.05 * total['FFR_Available'].max(),
...
# === Estética final ===
ax.set_title('Annual Total FFR by Scenario', fontsize=16, fontweight='bold')
ax.set_xlabel('Scenario', fontsize=12)
ax.set_ylabel('Total FFR (MW)', fontsize=12)
ax.spines[['top', 'right']].set_visible(False)
...
#     'AvailableFFR': [0, 2818222.41, 4336609.73, 0, 9049783.91, 9706870.68],
#     'RequiredFFR':  [0, 2449607.73, 3979297.48, 0, 2652662.08, 3919294.88]
# }
data = {
    'Scenario': ['EPI_NC', 'EPI_DC', 'EPI_SC', 'NZE_NC', 'NZE_DC', 'NZE_SC'],
    'AvailableFFR': [0, 2.82, 4.34, 0, 9.05, 9.71],
    'RequiredFFR':  [0, 2.45, 3.98, 0, 2.65, 3.92]
}
...
df['UnusedFFR'] = df['AvailableFFR'] - df['RequiredFFR']

# === Colores personalizados ===
color_required = '#2c3e50'  # azul oscuro elegante
alpha_required = 0.7        # opacidad deseada
color_unused = '#6A5ACD'   # azul claro profesional
alpha_unused = 0.7 
...
bars_required = ax.bar(
    df['Scenario'], df['RequiredFFR'],
    color=color_required, alpha=alpha_required,
    edgecolor='black', label='Required FFR'
)

bars_unused = ax.bar(
    df['Scenario'], df['UnusedFFR'],
    bottom=df['RequiredFFR'],
    color=color_unused, alpha=alpha_required,
    edgecolor='black', label='Unused Available FFR'
)

# === Etiquetas numéricas ===
for i in range(len(df)):
    total = df['AvailableFFR'].iloc[i]
    required = df['RequiredFFR'].iloc[i]
    
    # Etiqueta total (en la parte superior)
    ax.text(i, total + 0.1, f'{total:.2f}', ha='center', fontsize=12, color='black')
    
    # Etiqueta requerida (en la parte inferior de la barra)
    if required > 0:
        ax.text(i, required - 0.4, f'{required:.2f}', ha='center', fontsize=12, color='white')

# === Títulos y etiquetas ===
ax.set_title('Total FFR by Scenario', fontsize=16, fontweight='bold')
ax.set_ylabel('FFR (TWh)', fontsize=12)
ax.set_xlabel('Scenario', fontsize=12)
ax.yaxis.set_major_formatter(FuncFormatter(lambda x, _: f'{x:,.0f}'))
plt.xticks(rotation=45)
...
# === 2. Cumulative distribution FFR_Available by scenario (Boxplot) ===
...
sns.boxplot(data=df_concat, x='Escenario', y='FFR_Available',
            palette=colores_escenarios, fliersize=2, linewidth=1, ax=ax)

ax.set_title('Cumulative Distribution of System FFR by Scenario', fontsize=26, fontweight='bold')
ax.set_ylabel('System FFR (GWs)', fontsize=20)
...
sns.stripplot(data=df_concat, x='Escenario', y='FFR_Available',
...
#%%
# === 3. timeseries Daily Average System FFR with Range (Min–Max) by Scenario Group ===
import matplotlib.pyplot as plt
import pandas as pd
...
df_stats = df_concat.groupby(['Escenario', 'Fecha'])['FFR_Available'].agg(['mean', 'min', 'max']).reset_index()
...
for ax, (group_title, esc_list) in zip(axs, grupos_escenarios.items()):
    ...
    ax.set_ylabel('Daily System FFR (MW·s)', fontsize=12)
    ax.spines[['top', 'right']].set_visible(False)
...
# Título principal
fig.suptitle('Daily Average System FFR with Range (Min–Max) by Approach',
...
#%%
# === 3.1 (eje y fijo para todos de 0 a 30) timeseries Daily Average System FFR with Range (Min–Max) by Scenario Group ===
...
plt.show()
#%%
# === 3.2 (eje y fijo para todos y linea mas delgada) timeseries Daily Average System FFR with Range (Min–Max) by Scenario Group ===
...
for ax, (group_title, esc_list) in zip(axs, grupos_escenarios.items()):
    ...
    ax.set_ylabel('Daily System FFR (GW·s)', fontsize=26)
    ax.set_ylim(0, 35)
    ax.spines[['top', 'right']].set_visible(False)
...
#%%
# === 4. Mapa de calor: FFR_Available medio por zona y hora ===
import seaborn as sns
import matplotlib.pyplot as plt
...
# Escala común
vmin = df_concat['FFR_Available'].mean() - 2 * df_concat['FFR_Available'].std()
vmax = df_concat['FFR_Available'].mean() + 2 * df_concat['FFR_Available'].std()

# Dibujar heatmaps
for i, esc in enumerate(escenarios):
    ax = axs[i]
    df_esc = df_concat[df_concat['Escenario'] == esc]
    df_heatmap = df_esc.groupby('Hora')['FFR_Available'].mean().to_frame().T  # Solo 1 fila
    ...
    ax.set_ylabel('FFR_Available', fontsize=14)
    ax.tick_params(labelsize=12)
    ax.set_yticklabels([''])  # Quitar etiquetas del eje Y ya que es solo una fila

# Ajustar colorbar
cbar_ax.tick_params(labelsize=12)
cbar_ax.set_ylabel('FFR_Available (MWh)', fontsize=16, labelpad=20)

# Título global
fig.suptitle('FFR_Available medio por hora del día – por escenario',
...
plt.show()
#%%
# === 4.1 mejor todo agrupado Mapa de calor: FFR_Available medio por zona y hora ===
...
pivot_table = df_concat.groupby(['Approach', 'Escenario', 'Hora'])['FFR_Available'].mean().reset_index()
...
for i, approach in enumerate(approaches):
    ...
    heatmap_data = df_ap.pivot(index='Escenario', columns='Hora', values='FFR_Available')

    sns.heatmap(heatmap_data, ax=axs[i], cmap='viridis', annot=False,
                vmin=pivot_table['FFR_Available'].mean() - 2*pivot_table['FFR_Available'].std(),
                vmax=pivot_table['FFR_Available'].mean() + 2*pivot_table['FFR_Available'].std(),
                cbar=i==2, cbar_kws={'label': 'FFR_Available (MWs)'})
...
plt.suptitle('Average System FFR by Hour and Scenario – by Approach', fontsize=18, fontweight='bold')
...
df_plot = df_concat.groupby(['Approach', 'NCScenario', 'Hour'])['FFR_Available'].mean().reset_index()
...
vmin = df_plot['FFR_Available'].mean() - 2 * df_plot['FFR_Available'].std()
vmax = df_plot['FFR_Available'].mean() + 2 * df_plot['FFR_Available'].std()
...
for i, app in enumerate(approaches):
    ...
    for _, row in df_app.iterrows():
        color = cmap((row['FFR_Available'] - vmin) / (vmax - vmin))
        size = 200 + 1000 * (row['FFR_Available'] - vmin) / (vmax - vmin)
...
cbar.set_label('FFR_Available (GWs)', fontsize=14)
cbar.ax.tick_params(labelsize=12)

# Título global
fig.suptitle("System FFR Heat Map by Scenario and Approach", fontsize=20, fontweight='bold')
...
pivot_table = df_concat.groupby(['Approach', 'ScenarioBase', 'Hora'])['FFR_Available'].mean().reset_index()
...
for i, approach in enumerate(approaches):
    df_ap = pivot_table[pivot_table['Approach'] == approach]
    heatmap_data = df_ap.pivot(index='ScenarioBase', columns='Hora', values='FFR_Available')

    sns.heatmap(
        heatmap_data, ax=axs[i], cmap='viridis', annot=False,
        vmin=pivot_table['FFR_Available'].mean() - 2*pivot_table['FFR_Available'].std(),
        vmax=pivot_table['FFR_Available'].mean() + 2*pivot_table['FFR_Available'].std(),
        cbar=i == 2, cbar_kws={'label': 'FFR_Available (MWh)'}
    )
